...
class Cylinder {
    ...
    initBuffers(gl) {
        ...
        for (let i = 0; i < this.sides; i++) {
            ...
            allVertices.push(...bottomCenter, ...pt2_bottom, ...pt1_bottom); // Reversed order for correct face culling
            allUVs.push(
                0.5, 0.5,
                0.5 + 0.5 * Math.cos(angle2), 0.5 - 0.5 * Math.sin(angle2),
                0.5 + 0.5 * Math.cos(angle1), 0.5 - 0.5 * Math.sin(angle1)
    ...
    render(gl) {
        if (!this.vertexBuffer || !this.uvBuffer) {
            if (!this.initBuffers(gl)) { // Fallback initialization attempt
                 console.error("Cylinder.render: Buffers not initialized and fallback FAILED.");
                 return;
            }
        }
